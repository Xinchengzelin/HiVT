...
class LocalEncoder(nn.Module):
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        for t in range(self.historical_steps):
            # 从edge_index里得到subset 里node对应的edge_index
            data[f'edge_index_{t}'], _ = subgraph(subset=~data['padding_mask'][:, t], edge_index=data.edge_index)
            #source节点的position - target节点的position, source node是邻居j, 目标节点是i
            data[f'edge_attr_{t}'] = \
        ...
        if self.parallel:
            ...
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge(data[f'edge_index_{t}'], data[f'edge_attr_{t}']) #drop超过50m的障碍物
        ...
        out = self.temporal_encoder(x=out, padding_mask=data['padding_mask'][:, : self.historical_steps]) #(N,64)
        ...
        out = self.al_encoder(x=(data['lane_vectors'], out), edge_index=edge_index, edge_attr=edge_attr,
                              ...
                              traffic_controls=data['traffic_controls'], rotate_mat=data['rotate_mat']) #输出 （N,64）
        return out
...
class AAEncoder(MessagePassing):
    ...
    def forward(self,
                x: torch.Tensor, # (num_agents,2)
                t: Optional[int],
                edge_index: Adj,
                edge_attr: torch.Tensor,
                bos_mask: torch.Tensor, #(num_agents,)
        ...
        if self.parallel:
            ...
            if rotate_mat is None:
                ...
                center_embed = self.center_embed(torch.bmm(x.unsqueeze(-2), rotate_mat).squeeze(-2)) # 输入(num_agents,2), 输出（num_agents, 64）
            #如果对应的agent是bos，选择学习参数对应的bos_token,否则就是center_embed，学习的位置编码
        ...
        center_embed = center_embed + self._mha_block(self.norm1(center_embed), x, edge_index, edge_attr, rotate_mat,
                                                      size)
        center_embed = center_embed + self._ff_block(self.norm2(center_embed)) # norm2: LN
        return center_embed
    ...
    def message(self,
        ...
        if rotate_mat is None:
            ...
            if self.parallel:
                ...
                center_rotate_mat = rotate_mat[edge_index[1]] #edge_index[1] target node, 0是source node, PyG默认source->target
        ...
        query = self.lin_q(center_embed_i).view(-1, self.num_heads, self.embed_dim // self.num_heads) #(310,8,8) 310 edge_index的长度
        ...
        alpha = (query * key).sum(dim=-1) / scale # PyG的softmax  (310，8)  (q*k).sum(dim=-1)计算的是Q@K^T的对角线元素
        alpha = softmax(alpha, index, ptr, size_i) # （310，8）
        alpha = self.attn_drop(alpha) # 和V相乘之前，dropout
        return value * alpha.unsqueeze(-1) #TODO 结果 (310,8,8)  为什么不是矩阵乘法？

    def update(self,
               inputs: torch.Tensor,
               center_embed: torch.Tensor) -> torch.Tensor:
        inputs = inputs.view(-1, self.embed_dim) #message结果就是update的输入 (N,64)
        gate = torch.sigmoid(self.lin_ih(inputs) + self.lin_hh(center_embed)) #lin_ih, lin_hh都是线性投影 (N,64)
    ...
    def _mha_block(self,
        ...
        center_embed = self.out_proj(self.propagate(edge_index=edge_index, x=x, center_embed=center_embed,
                                                    edge_attr=edge_attr, rotate_mat=rotate_mat, size=size))#最后是一个同维度线性映射
...
class TemporalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.padding_token = nn.Parameter(torch.Tensor(historical_steps, 1, embed_dim))#这个应该怎么理解？
        ...
        self.register_buffer('attn_mask', attn_mask) #Adds a buffer to the module.
    ...
    def forward(self,
        ...
        x = torch.where(padding_mask.t().unsqueeze(-1), self.padding_token, x) #(20,N,64)
        expand_cls_token = self.cls_token.expand(-1, x.shape[1], -1)
        x = torch.cat((x, expand_cls_token), dim=0)
        x = x + self.pos_embed
        out = self.transformer_encoder(src=x, mask=self.attn_mask, src_key_padding_mask=None) #(21,N,64)
        return out[-1]  # [N, D]

    @staticmethod
    def generate_square_subsequent_mask(seq_len: int) -> torch.Tensor:
        mask = (torch.triu(torch.ones(seq_len, seq_len)) == 1).transpose(0, 1)  # 不transpose，下面直接改变填的值也可以
        mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0)) #上三角（不包括对角线）都是-inf,下面都是0
        return mask
...
class ALEncoder(MessagePassing):
    ...
    def forward(self,
                x: Tuple[torch.Tensor, torch.Tensor], # data['lane_vectors'], out
                edge_index: Adj,
                edge_attr: torch.Tensor,
        ...
        x_actor = x_actor + self._mha_block(self.norm1(x_actor), x_lane, edge_index, edge_attr, is_intersections,
                                            turn_directions, traffic_controls, rotate_mat, size)# 注意 进_mha_block之前有个LN
        x_actor = x_actor + self._ff_block(self.norm2(x_actor))
        return x_actor

    def message(self,
                edge_index: Adj,
                x_i: torch.Tensor, # x_actor
                x_j: torch.Tensor, # x_lane
                edge_attr: torch.Tensor, # data['lane_actor_vectors']drop_edge后的结果
                is_intersections_j,
                turn_directions_j,
                traffic_controls_j,
                rotate_mat: Optional[torch.Tensor],
        ...
        if rotate_mat is None:
            ...
            x_j = self.lane_embed([torch.bmm(x_j.unsqueeze(-2), rotate_mat).squeeze(-2),
                                  ...
                                  [self.is_intersection_embed[is_intersections_j],
                                   self.turn_direction_embed[turn_directions_j],
                                   self.traffic_control_embed[traffic_controls_j]]) # 输出x_j（num_points, 64）
